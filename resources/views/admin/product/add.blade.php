...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <form method="post" id="data-form" action="{{ route('product.store') }}" enctype="multipart/form-data"
            ...
            <div class="col-lg-7 col-md-7 col-sm-12">
                ...
                <div class="card mb-30">
                    ...
                    <div class="card-body">
                        @include('admin.product.tabs.productseo')
                    </div>
                </div>
...
@push('js')
    ...
    <script>
        ...
        function appendToOptionTable(tableId, optionID, optionType) {
            ...
            let renderHTML = '<tr class="tr_option_clone"><td><input type="text" placeholder="Enter Label" name="option[' +
                optionID + '][label][]" class="form-control" type="text" /></td>';
            renderHTML += '<td><input type="number"   name="option[' + optionID +
                '][price][]" class="form-control" type="text" placeholder="Enter Price" /></td>';
            if (optionType == 'Color') {
                renderHTML += '<td><input type="text" placeholder="Enter Color Name or Code" name="option[' + optionID +
                    '][color_code][]" class="form-control" type="text" /></td>';
            }
            renderHTML +=
                '<td><button class="btn btn-danger" type="button" type="button" id="DeleteOptionButton" ><icon class="fa fa-minus" /></button>  </td></tr>'
        ...
        function setOption(val) {
            console.log("otpion arr ", optionArr)
            if ($(val).val() != '') {
                var addOption = false;
                ...
                if (addOption) {
                    ...
                    $('.appendTab').append('<button type="button" onclick="optionClick(' + $(val).val() +
                        ')" class="btn btn-primary" id="btn' + $(val).val() +
                        '" ><span style="margin-right:10px;" onclick="removeOption(' + $(val).val() +
                        ')"> <i class="fas fa-minus-square"></i> </span> ' + $(val).find('option:selected').text() +
                        '</button></br>')
