            ...
            <div class="tab-pane @if ($key == 0) active @endif" id="language{{ $language->id }}">
                ...
                <div class="col-md-12 form-group{{ $errors->has('multilanguage.*.name') ? ' has-danger' : '' }}">
                    ...
                    <input type="text" name="multilanguage[{{ $language->id }}][name]"
                        ...
                        onkeyup="onProductNameChange('{{ $language->id }}')">
                ...
                <div class="chatGPTButton{{ $language->id }} mx-3 mb-4 d-none">
                    <button class="btn-chatgpt btn btn-md btn-dark"
                        onclick="getContentFromChatGPT('{{ $language->id }}')"><i class="fas fa-robot"></i> Get Content
                        From ChatGPT</button>
                </div>
                ...
                <div class="col-md-12 form-group{{ $errors->has('short_description') ? ' has-danger' : '' }}">
                    ...
                    <textarea name="multilanguage[{{ $language->id }}][short_description]"
                        class="form-control short_description{{ $language->id }}" placeholder="{{ __('Product Short Description') }}"
