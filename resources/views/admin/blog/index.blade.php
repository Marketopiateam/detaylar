            ...
            <div class="header-body">
                <div class="row align-items-center py-4">
                    ...
                    <div class="col-lg-6 col-5 text-right">
                        <a href="{{ route('blog.add') }}" class="btn btn-lg btn-neutral fade-class"><i
                            ...
                            <tbody class="list">
                                @forelse($data as $record)
                                    <tr>
                                        <td>
                                            @if ($record->image)
                                                <img class="avatar user-thumb rounded-circle"
                                                    src="{{ asset("$record->image") }}" alt="avatar"
                                                    style="height:60px;width:60px">
                                            @else
                                                <img class="avatar user-thumb rounded-circle"
                                                    src="{{ asset('assets/images/media/image_not_found.png') }}"
                                                    alt="avatar" style="height:60px;width:60px">
                                            @endif
                                        </td>
                                        <td>{{ $record->adminblogDescription?->title }}</td>
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                                    <a class="dropdown-item " type="button"
                                                        href="{{ route('blog.edit', ['id' => $record->id]) }}">Edit</a>
                                                    <a class="dropdown-item deleteData" type="button"
                                                        href="javascript:void(0)"
                                                        data-url="{{ route('blog.delete', ['id' => $record->id]) }}">Delete</a>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
