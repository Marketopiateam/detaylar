...
class ProductController extends Controller
{
  ...
  public function index(Request $request)
  {
    ...
    $records = Product::select('id', 'image', 'category_id', 'model', 'price', 'location', 'quantity', 'sort_order', 'status')
      ->with('adminproductDescription:name,id,product_id', 'admincategory:name,category_id')
      ->when($name != '' || $model != '' || $quantity != '' || $status != '', function ($q) use ($name, $model, $quantity, $status) {
        ...
        $q->whereHas('adminproductDescription', function ($q) use ($name) {
  ...
  {
    $data['category'] =  Category::getAdminActivePluck();
    $data['stock_status'] =  StockStatus::getActivePluck();
    $data['manufacturer'] =  Manufacturer::getActivePluck();
    $data['tax_rate'] =  TaxClass::getActivePluck();
    $data['lenght_class'] =  LengthClass::getActivePluck();
    $data['weight_class'] =  WeightClass::getActivePluck();
    $data['pluckProducts'] =  Product::with('adminproductDescription:name,id,product_id')->has('adminproductDescription')->get();
    $data['product_options'] =  ProductOption::with('adminproductoptionDescription')->get();
    $data['attributeArray'] =  ProductAttributeGroup::with('AdminProductAttributeGroupDescription:language_id,attribute_group_id,name', 'adminrelationAttributes:language_id,name,id,group_id')->select('id')->get();
  ...
  public function store(Request $request)
  {
    ...
    $product->weight_class_id  = $request->weight_class_id ? $request->weight_class_id : 0;
    ...
    $buildMultiLanguage = $description->buildMultiLang($product->id, $request->multilanguage);
    $description->upsert($buildMultiLanguage, ['product_id', 'short_description', 'description', 'language_id', 'name', 'meta_title', 'meta_description', 'meta_keyword']);
  ...
  public function edit($id)
  {
    ...
    $data['productOptions'] = StoreProductOption::with('adminproductoptionDescription')
      ...
      foreach ($request->product_image['sort_order_image'] as $key => $value) {
        if (isset($request->product_image['id'][$key])) {
          ...
          if (isset($request->product_image['image'][$key])) {
            ...
            $productImage->image = "uploads/product/$productImage->image";
          }
